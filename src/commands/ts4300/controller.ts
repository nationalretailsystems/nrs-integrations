...
import * as converterm from 'src/interfaces/t4300mov';
...
export const postCartMove: ECCHandlerFunction = async (reqkey, data, ecc) => {
    logger.debug(`Received Cartridge Move request`, { reqkey, data });
    // Get parameters from incoming data buffer
    const reqFields = converterm.convertTapeMovReqToObject(data);
    // Call web service
    let result;
    let nextReqKey = reqkey;
