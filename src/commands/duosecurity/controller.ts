...
import { duo_api } from '@duosecurity/duo_api/lib';
import axios from 'axios';
import config from 'config';
import { DateTime } from 'luxon';
import createLogger from 'src/services/logger';
import * as converter from 'src/interfaces/duosecurity';

const logger = createLogger('commands/duosecurity');
const { duosecurity } = config;


const duo_api = require('duo_api');
const client = new duo_api.Cient(
    duosecurity.integrationKey, 
    duosecurity.secretKey, 
...
export const preauth: ECCHandlerFunction = async (reqkey, data, ecc) => {
