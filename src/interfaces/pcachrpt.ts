...
export function convertObjectToPCRcvRpt(dataIn: PCRcvRpt): string {
  ...
  dataOut += dataIn?.overall?.debitamount?.toFixed(5)?.substring(0, 17)?.padEnd(17) ?? "0".substring(0, 17).padEnd(17);
  dataOut += dataIn?.overall?.creditamount?.toFixed(5)?.substring(0, 17)?.padEnd(17) ?? "0".substring(0, 17).padEnd(17);
