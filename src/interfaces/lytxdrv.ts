...
/**
 * Convert DrvReq record to TypeScript object
 */
export function convertDrvReqToObject(dataIn: string): DrvReq {
  const dataOut: any =   {
  
    };
  let pos: number = 0;

  dataOut.driver = dataIn.substring(pos, pos + 10).trimEnd();
  pos += 10;
...
/**
 * Convert JavaScript object to DrvRes record
 */
export function convertObjectToDrvRes(dataIn: DrvRes): string {
  let dataOut: string = "";
