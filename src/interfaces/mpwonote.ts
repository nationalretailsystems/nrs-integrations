...
export function convertObjectToResNotes(dataIn: ResNotes): string {
    ...
    for (let i: number = 0; i < 10; ++i) {
        dataOut +=
            dataIn?.notesdata[i]?.noteKey?.toFixed()?.substring(0, 11)?.padEnd(11) ?? '0'.substring(0, 11).padEnd(11);
        dataOut +=
            dataIn?.notesdata[i]?.workOrderNumber?.toFixed()?.substring(0, 11)?.padEnd(11) ??
            '0'.substring(0, 11).padEnd(11);
        dataOut +=
            dataIn?.notesdata[i]?.workOrderKey?.toFixed()?.substring(0, 11)?.padEnd(11) ??
            '0'.substring(0, 11).padEnd(11);
        ...
        dataOut +=
            dataIn?.notesdata[i]?.isPrivateNote !== undefined ? (dataIn?.notesdata[i]?.isPrivateNote ? '1' : '0') : '0';
